...
export const CreepyButton = ({
  ...
  return (
    <button
      className={cn(
        "relative min-w-[9em] rounded-full bg-ink cursor-pointer outline-none select-none group tap-highlight-transparent",
        "focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-ink",
        className
      )}
      onClick={onClick}
      onMouseMove={(e) => {
    ...
    >
      ...
      >
        ...
        >
          <span
            className="absolute top-1/2 left-1/2 w-[0.375em] h-[0.375em] bg-ink rounded-full transition-transform duration-75 ease-out"
            style={pupilStyle}
          />
        </motion.span>
        <motion.span
      ...
      <motion.span
        className={cn(
          "absolute inset-0 block rounded-full bg-white text-ink font-medium tracking-tight text-[13px]",
          "shadow-[inset_0_0_0_0.09em_rgba(0,0,0,0.08)]",
      ...
      <span className="block opacity-0 px-6 py-3 text-[13px] font-medium tracking-tight min-w-[9em]">
        {children}
      </span>
    </button>
  );
